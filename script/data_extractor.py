...
class DataExtractor():
    ...
    def get_columns_and_rows(self,file_path)->tuple:
        try:
            ...
            try:
                self.error(f"Failed to read data: {e}")
            except:
                pass
            sys.exit(1)
        ...
        try:
            ...
            for row in all_data:
                try:
                    ...
                    # the try excepts here are for the airflow
                    try:
                        self.error(f"Failed preparing data for pandas at row {row}: {e}")
                    except:
                        pass
            ...
            # the try excepts here are for the airflow
            try:
                self.error(f"Failed to prepare data for pandas: {e}")
            except:
                pass
    ...
    def prepare_data_frame(self,trajectory_data:tuple,timed_vehicle_data:tuple):
        ...
        try:
            ...
            # the try excepts here are for the airflow
            try:
                self.error(f"Failed to prepare data frame: {e}")
            except:
                pass
    ...
    def extract_data(self,file_name:str,return_json=False)->pd.DataFrame:
        try:
            ...
            print(e)
            try:
                self.error(f"Failed to extract data: {e}")
            except:
                pass
